...
<head>
    ...
    $brand_rs = Database::search("SELECT * FROM `brand` WHERE `id`='" . $_GET["id"] . "'AND `status_id`='1'");
    $brand_data = $brand_rs->fetch_assoc();

    $category_rs = Database::search("SELECT * FROM `category` INNER JOIN `category_has_brand` ON `category`.`id` = `category_has_brand`.`category_id` WHERE `category_has_brand`.`brand_id` = '" . $brand_data["id"] . "'");
    $category_data = $category_rs->fetch_assoc();

    ?>
    <title>Dig Shop | <?php echo $category_data["category_name"] . "-" . $brand_data["brand_name"]; ?></title>
    ...
    <div class="container-fluid background background-1">
        <div class="row">
            ...
            <div class="col-12 mt-3 mb-3">
                <a href="#" class="text-decoration-none link-dark fs-3 fw-bold"><?php echo  $category_data["category_name"] . " - " . $brand_data["brand_name"]; ?> Products</a>
            </div>
                ...
                <div class="row border border-primary border-2 rounded-3">
                    ...
                    <div class="col-12">
                        <div class="row justify-content-center gap-3">
                            ...
                            $query = "SELECT  product.id AS pid FROM `product` INNER JOIN `category_has_brand` ON product.category_has_brand_id=category_has_brand.id WHERE `brand_id`='" . $brand_data["id"] . "' AND `product`.`status_id`='1' ORDER BY `product`.`date_time` DESC";
