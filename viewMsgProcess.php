<?php

session_start();
require "connection.php";

$recever_mail = $_SESSION["user"]["email"];
$sender_mail = $_GET["e"];

$msg_rs = Database::search("SELECT * FROM `chat` WHERE `from`='" . $sender_mail . "' OR `to`='" . $sender_mail . "'");
$msg_num = $msg_rs->num_rows;

$sender_rs = Database::search("SELECT * FROM `user` WHERE `email`='" . $sender_mail . "'");
$sender_data = $sender_rs->fetch_assoc();

?>

<div class="col-12" style="background-color:  #F2F4F4;">
    <div class="row">
        <div class="col-6 text-start">

            <?php

            $img_rs = Database::search("SELECT * FROM `profile_image` WHERE `user_email`='" . $sender_mail . "'");
            $img_data = $img_rs->fetch_assoc();

            if (isset($img_data["path"])) {

            ?>
                <img src="<?php echo $img_data["path"]; ?>" width="30px" height="30px" class="rounded-circle mt-2 mb-1 ms-3" />

            <?php

            } else {

            ?>

                <img src="resource/user_images/user-1.svg" width="30px" height="30px" class="rounded-circle mt-2 mb-1 ms-3" />
            <?php

            }

            ?>

            <span class="mt-2 ms-2"><?php echo $sender_data["fname"] . " " . $sender_data["lname"]; ?></span>
            <p class="d-none" id="rmail"><?php echo $sender_mail; ?></p>


        </div>
        <div class="col-6 text-end">
            <div class="dropdown">
                <button class="btn btn-light rounded-circle mt-1 mb-1 me-1" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                    <i class="bi bi-three-dots-vertical"></i>
                </button>
                <ul class="dropdown-menu">
                    <li><a class="dropdown-item" href="#" onclick="deleteChat('<?php echo $sender_mail; ?>');">Clear Chat</a></li>
                </ul>
            </div>
        </div>
    </div>
</div>

<div class="row px-4 pt-3 text-white chat_box bg-transparent">

    <?php

    for ($x = 0; $x < $msg_num; $x++) {
        $msg_data = $msg_rs->fetch_assoc();

        if ($msg_data["from"] == $sender_mail && $msg_data["to"] == $recever_mail) {

            $user_rs = Database::search("SELECT * FROM `user` WHERE `email`='" . $msg_data["from"] . "'");
            $user_data = $user_rs->fetch_assoc();

            $img_rs = Database::search("SELECT * FROM `profile_image` WHERE `user_email`='" . $msg_data["from"] . "'");
            $img_data = $img_rs->fetch_assoc();

            Database::iud("UPDATE `chat` SET `status`='1' WHERE `from`='" . $sender_mail . "'");


    ?>

            <!-- sender -->
            <div class="justify-content-start d-flex">
                <div class="media m-0 w-50">
                    <div class="media-body">
                        <div class="bg-warning rounded-5 py-2 px-3 text-end">
                            <p class="mb-0 fw-bold text-black-50"> <?php echo $msg_data["content"]; ?></p>
                        </div>
                        <p class="small fw-bold text-black-50 me-3 text-end"><?php echo $msg_data["date_time"]; ?></p>
                    </div>
                </div>
            </div>
            <!-- sender -->

        <?php

        } else if ($msg_data["to"] == $sender_mail && $msg_data["from"] == $recever_mail) {

        ?>

            <!-- receiver -->
            <div class="justify-content-end d-flex">
                <div class="media m-0 w-50">
                    <div class="media-body">
                        <div class="bg-primary rounded-5 py-2 px-3 text-start">
                            <p class="mb-0 text-white"><?php echo $msg_data["content"]; ?></p>
                        </div>
                        <div class="col-12">
                            <div class="row">
                                <div class="col-6">
                                    <p class="small fw-bold text-black-50 ms-3"><?php echo $msg_data["date_time"]; ?></p>
                                </div>
                                <div class="col-6 text-end">

                                    <?php

                                    if ($msg_data["status"] == 1) {

                                    ?>

                                        <p class="me-3 text-secondary"><i class="bi bi-check-all"></i></p>

                                    <?php

                                    } elseif ($msg_data["status"] == 0) {

                                    ?>

                                        <p class="me-3 text-secondary"><i class="bi bi-check"></i></p>

                                    <?php

                                    }

                                    ?>

                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <!-- receiver -->

    <?php
        }
    }

    ?>
</div>

<div class="col-12 px-2" style="background-color:  #F2F4F4;">
    <div class="row g-2">
        <div class="input-group mt-3 mb-3">
            <input type="text" id="msg_text" class="col-10 me-3 form-control border-0 rounded-5" style="background-color:  #CCD1D1;" placeholder="Type a message ..." aria-describedby="send_btn">
            <button class="btn btn-warning rounded-circle" id="send_btn" onclick="send_msg()"><i class="bi bi-send-fill"></i></button>
        </div>
    </div>
</div>