...
$sender_rs = Database::search("SELECT * FROM `user` WHERE `email`='" . $sender_mail . "'");
...
<div class="col-12" style="background-color:  #F2F4F4;">
    <div class="row">
        <div class="col-6 text-start p-2">
            ...
            $img_rs = Database::search("SELECT * FROM `profile_image` WHERE `user_email`='" . $sender_mail . "'");
            $img_data = $img_rs->fetch_assoc();

            if (isset($img_data["path"])) {

            ?>
                <img src="<?php echo $img_data["path"]; ?>" width="30px" height="30px" class="rounded-circle mt-2 mb-1 ms-3" />
            ...
            ?>
                ...
                <img src="resource/user_images/user-1.svg" width="30px" height="30px" class="rounded-circle mt-2 mb-1 ms-3" />
            <?php
        ...
        <div class="col-6 text-end p-2">
            <div class="dropdown">
                ...
                <ul class="dropdown-menu">
                    <li><a class="dropdown-item" href="#" onclick="clearChat('<?php echo $sender_mail; ?>');">Clear Chat</a></li>
                </ul>
            </div>
        </div>
    </div>
...
<div class="row px-4 pt-3 text-white chat_box bg-transparent pt-lg-5 pb-lg-4">
    ...
    for ($x = 0; $x < $msg_num; $x++) {
        ...
        if ($msg_data["type_id"] == "1") {

            $user_rs = Database::search("SELECT * FROM `user` WHERE `email`='" . $msg_data["user_email"] . "'");
            $user_data = $user_rs->fetch_assoc();

            $img_rs = Database::search("SELECT * FROM `profile_image` WHERE `user_email`='" . $msg_data["user_email"] . "'");
            $img_data = $img_rs->fetch_assoc();

            Database::iud("UPDATE `admin_chat` SET `status`='4' WHERE `user_email`='" . $sender_mail . "' AND `type_id`='1'");
...
<div class="col-12 p-1 px-3" style="background-color:  #F2F4F4;">
    <div class="row g-2">
        <div class="input-group mt-3 mb-2">
            ...
            <button class="btn btn-warning rounded-circle" id="send_btn" onclick="send_user_msg()"><i class="bi bi-send-fill"></i></button>
        </div>
    </div>
</div>